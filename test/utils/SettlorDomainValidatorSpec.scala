...
class SettlorDomainValidatorSpec extends BaseSpec with DataExamples {
  ...
  "deceasedSettlorDobIsNotFutureDate" should {
    ...
    "return no validation errors when there is a deed of variation, in addition to a will trust" in {
      val deedOfVariationTrust =
        getJsonValueFromFile("deed-of-variation-in-addition-to-a-will-trust-both-settlors.json").as[Registration]
    ...
    "return no validation errors when there is a deed of variation, in addition to a will trust, without a living settlor" in {
      val deedOfVariationTrust =
        getJsonValueFromFile("deed-of-variation-in-addition-to-a-will-trust-no-living-settlor.json").as[Registration]
    ...
    "return validation error when there is a deed of variation, in addition to a will trust, without a deceased settlor" in {
      val deedOfVariationTrust =
        getJsonValueFromFile("deed-of-variation-in-addition-to-a-will-trust-no-deceased.json").as[Registration]

      SUT(
        deedOfVariationTrust
      ).validateSettlor.get.message                       mustBe "Deed of Variation Trust or Family Arrangement, Addition to the will trust, must have a deceased settlor"
    ...
    "return a validation error where there is a deed of variation, not an addition to a will trust and there is no living settlor" in {
      val deedOfVariationTrust =
        getJsonValueFromFile("deed-of-variation-replaced-the-will-no-living.json").as[Registration]

      SUT(
        deedOfVariationTrust
      ).validateSettlor.get.message                       mustBe "Deed of Variation Trust or Family Arrangement, Replaced the will trust, must have a living settlor"
      BusinessValidation.check(deedOfVariationTrust).size mustBe 1
    }
    ...
    "return a validation error where there is a deed of variation, not an addition to a will trust and there is a deceased settlor" in {
      val deedOfVariationTrust =
        getJsonValueFromFile("deed-of-variation-replaced-the-will-deceased.json").as[Registration]

      SUT(
        deedOfVariationTrust
      ).validateSettlor.get.message                       mustBe "Deed of Variation Trust or Family Arrangement, Replaced the will trust, must not have a deceased settlor"
      BusinessValidation.check(deedOfVariationTrust).size mustBe 1
    }
    ...
    "return validation error when there is no deceased settlor for will trust." in {
      val willTrust = getJsonValueFromString(trustWithValues(typeOfTrust = TypeOfTrust.Will)).validate[Registration].get
    ...
    "return validation error when individual settlor has same NINO" in {
      ...
      response.flatten.zipWithIndex.map { case (error, index) =>
    ...
    "return validation error when individual settlor date of birth is future date." in {
      val employmentTrust =
        getJsonValueFromString(trustWithValues(settlorDob = "2050-01-01")).validate[Registration].get
    ...
    "return validation error when company settlor has same UTR" in {
      val employmentTrust =
        getJsonValueFromString(trustWithValues(settlorUtr = "1234561234")).validate[Registration].get
    ...
    "return validation error when company settlor utr is trust utr" in {
      val employmentTrust =
        getJsonValueFromString(trustWithValues(settlorUtr = "5454541615")).validate[Registration].get
    ...
    "return None when company settlor utr is not trust utr" in {
      val employmentTrust =
        getJsonValueFromString(trustWithValues(settlorUtr = "5454541616")).validate[Registration].get
      val response        = SUT(employmentTrust).companySettlorUtrIsNotTrustUtr
