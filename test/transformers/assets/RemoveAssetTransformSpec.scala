...
class RemoveAssetTransformSpec extends AnyFreeSpec with ScalaFutures with MockitoSugar {
  ...
  private val inputJson = buildInputJson(
    Seq(
      assetJson("One"),
      assetJson("Two"),
      assetJson("Three")
  ...
  "the remove asset normal transform must" - {
    ...
    "remove an asset from the list that is returned to the frontend" in {
      ...
      val OUT = ComposedDeltaTransform(
        Seq(RemoveAssetTransform(Some(1), Json.obj(), LocalDate.of(2018, 4, 21), "nonEEABusiness"))
      )
    ...
    "not affect the document if the index is too high" in {
      ...
      val OUT = ComposedDeltaTransform(
        Seq(RemoveAssetTransform(Some(10), Json.obj(), LocalDate.of(2018, 4, 21), "nonEEABusiness"))
      )
    ...
    "remove the section if the last asset in that section is removed" in {
      ...
      val transforms = Seq(
        RemoveAssetTransform(
          Some(0),
          assetJson("One", None, withLineNo = false),
      ...
      OUT.applyTransform(inputJson) match {
        case JsSuccess(value, _) =>
          value.transform((JsPath() \ "details" \ "trust" \ "assets" \ "nonEEABusiness").json.pick).isError mustBe true
        case _                   => fail("Transform failed")
      }
    }
  }
  ...
  "the remove asset declaration transform must" - {
    ...
    "when non-EEA business asset" - {
      "must set an end date" in {
        ...
        val transforms    =
          Seq(RemoveAssetTransform(Some(1), assetJson("Two"), LocalDate.of(2018, 4, 21), "nonEEABusiness"))
    ...
    "ignore an asset that was added then removed" in {
      ...
      val transforms    = Seq(
        ...
        RemoveAssetTransform(
          Some(3),
          assetJson("Two", None, withLineNo = false),
          LocalDate.of(2018, 4, 21),
          "nonEEABusiness"
        )
      )
