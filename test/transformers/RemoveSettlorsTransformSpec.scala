...
class RemoveSettlorsTransformSpec extends FreeSpec with MustMatchers with ScalaFutures with MockitoSugar {
  ...
  "Remove Settlor Transforms should round trip through JSON as part of Composed Transform" in {
    val OUT = ComposedDeltaTransform(Seq(
      RemoveSettlorsTransform(56, settlorJson("Blah Blah Blah"), LocalDate.of(1563, 10, 23), "settlor"),
      RemoveSettlorsTransform(12, settlorJson("Foo"), LocalDate.of(2317, 12, 21),  "settlorCompany")
    ))
  ...
  "the remove individual settlor normal transform must" - {
    "remove an individual settlor from the list that is returned to the frontend" in {
      ...
      val OUT = ComposedDeltaTransform(Seq(RemoveSettlorsTransform(1, Json.obj(), LocalDate.of(2018, 4, 21), "settlor")))
    ...
    "not affect the document if the index is too high" in {
      ...
      val OUT = ComposedDeltaTransform(Seq(RemoveSettlorsTransform(10, Json.obj(), LocalDate.of(2018, 4, 21), "settlor")))
  ...
  "the remove business settlor declaration transform must" - {
    ...
    "not affect the document if the index is too high" in {
      ...
      val OUT = ComposedDeltaTransform(Seq(RemoveSettlorsTransform(10, Json.obj(), LocalDate.of(2018, 4, 21), "settlorCompany")))
    ...
    "not affect the document if the index is too low" in {
      ...
      val OUT = ComposedDeltaTransform(Seq(RemoveSettlorsTransform(-1, Json.obj(), LocalDate.of(2018, 4, 21), "settlorCompany")))
