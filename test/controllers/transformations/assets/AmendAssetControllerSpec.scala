...
class AmendAssetControllerSpec extends FreeSpec with MockitoSugar with ScalaFutures with MustMatchers
    ...
    "money asset" - {
      ...
      "must add a new amend transform" in {
        ...
        when(mockTransformationService.getTransformedTrustJson(any(), any())(any()))
          .thenReturn(Future.successful(buildInputJson(originalAsset.toString, Seq(Json.toJson(originalAsset)))))
        ...
        val transform = AmendAssetTransform(index, Json.toJson(amendedAsset), Json.toJson(originalAsset), endDate, originalAsset.toString)
