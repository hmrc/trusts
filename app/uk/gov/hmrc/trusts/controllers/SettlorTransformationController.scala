  ...
  def amendIndividualSettlor(utr: String, index: Int): Action[JsValue] = identify.async(parse.json) {
    implicit request => {
      request.body.validate[Settlor] match {
        ...
        case JsError(errors) =>
          logger.warn(s"[SettlorTransformationController][amendIndividualSettlor][Session ID: ${request.sessionId}]" +
  ...
  def addIndividualSettlor(utr: String): Action[JsValue] = identify.async(parse.json) {
    implicit request => {
      request.body.validate[Settlor] match {
        ...
        case JsError(errors) =>
          logger.warn(s"[SettlorTransformationController][addIndividualSettlor][Session ID: ${request.sessionId}]" +
            s" Supplied json could not be read as a Settlor - $errors")
          Future.successful(BadRequest)
      }
    }
  ...
  def addBusinessSettlor(utr: String): Action[JsValue] = identify.async(parse.json) {
    implicit request => {
      request.body.validate[SettlorCompany] match {
        ...
        case JsError(errors) =>
          logger.warn(s"[SettlorTransformationController][addBusinessSettlor][Session ID: ${request.sessionId}]" +
            s" Supplied json could not be read as a Settlor - $errors")
          Future.successful(BadRequest)
      }
    }
  ...
  def amendBusinessSettlor(utr: String, index: Int): Action[JsValue] = identify.async(parse.json) {
    implicit request => {
      request.body.validate[SettlorCompany] match {
        ...
        case JsError(errors) =>
          logger.warn(s"[SettlorTransformationController][amendBusinessSettlor][Session ID: ${request.sessionId}]" +
  ...
  def removeSettlor(utr: String): Action[JsValue] = identify.async(parse.json) {
    implicit request => {
      request.body.validate[RemoveSettlor] match {
        ...
        case JsError(errors) =>
          logger.warn(s"[SettlorTransformationController][removeSettlor][Session ID: ${request.sessionId}]" +
  ...
  def amendDeceasedSettlor(utr: String): Action[JsValue] = identify.async(parse.json) {
    implicit request =>
      request.body.validate[AmendDeceasedSettlor] match {
        ...
        case JsError(errors) =>
          logger.warn(s"[SettlorTransformationController][amendDeceasedSettlor][Session ID: ${request.sessionId}]" +
