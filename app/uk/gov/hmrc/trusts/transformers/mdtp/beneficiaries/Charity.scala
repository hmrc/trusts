...
object Charity {
  ...
  private val path = JsPath \ 'details \ 'trust \ 'entities \ 'beneficiary \ 'charity
  ...
  def transform(response : JsValue) : Reads[JsObject] = {
    response.transform(path.json.pick).fold(
      ...
      beneficiaries => {
        ...
        val beneficiariesUpdated = JsArray(beneficiaries.as[List[DisplayTrustCharityType]].map {
          beneficiary =>
            Json.toJson(beneficiary)(DisplayTrustCharityType.writeToMaintain)
        })
