...
class RosmPatternServiceImpl @Inject() (trustsService: TrustsService, taxEnrolmentService: TaxEnrolmentsService)(
  ...
  override def setSubscriptionId(trn: String, taxable: Boolean)(implicit
    hc: HeaderCarrier
    ...
    val result = for {
      ...
      taxEnrolmentSubscriberResponse <- TrustEnvelope.fromFuture(
                                          taxEnrolmentService.setSubscriptionId(
                                            subscriptionIdSuccessResponse.subscriptionId,
                                            taxable,
                                            trn
                                          )
                                        )
    } yield taxEnrolmentSubscriberResponse

    result.value.map {
      case Right(response)                                    => Right(response)
      case Left(error: ServerError) if error.message.nonEmpty =>
        logger.warn(
          s"[$className][setSubscriptionId][Session ID: ${Session.id(hc)}] failed to set subscriptionId - ${error.message}"
        )
        Left(error)
      case Left(_)                                            =>
  ...
  override def enrolAndLogResult(trn: String, affinityGroup: AffinityGroup, taxable: Boolean)(implicit
    hc: HeaderCarrier
  ): TrustEnvelope[TaxEnrolmentSubscriberResponse] = EitherT {
    affinityGroup match {
      case AffinityGroup.Organisation =>
        setSubscriptionId(trn, taxable).value.map {
          case Right(TaxEnrolmentSuccess) =>
            logger.info(
              s"[$className][enrolAndLogResult][Session ID: ${Session.id(hc)}] " +
...
trait RosmPatternService {
  ...
  def setSubscriptionId(trn: String, taxable: Boolean)(implicit
    hc: HeaderCarrier
  ): TrustEnvelope[TaxEnrolmentSubscriberResponse]

  def enrolAndLogResult(trn: String, affinityGroup: AffinityGroup, taxable: Boolean)(implicit
    hc: HeaderCarrier
  ): TrustEnvelope[TaxEnrolmentSubscriberResponse]
