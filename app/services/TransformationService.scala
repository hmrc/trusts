...
@Singleton
class TransformationService @Inject() (
  ...
  def getTransformedTrustJson(identifier: String, internalId: String, sessionId: String)(implicit
    hc: HeaderCarrier
  ): TrustEnvelope[JsObject] = EitherT {
    getTransformedData(identifier, internalId, sessionId).value.flatMap {
      ...
      case Left(ServerError(message)) if message.nonEmpty =>
        logger.warn(
          s"[$className][getTransformedTrustJson][SessionId: $sessionId] failed to get transformed trust data - $message"
        )
        Future.successful(Left(ServerError()))
      case Left(_)                                        =>
        logger.warn(
          s"[$className][getTransformedTrustJson][SessionId: $sessionId] failed to get transformed trust data."
        )
        Future.successful(Left(ServerError()))
    }
  }

  def getTransformedData(identifier: String, internalId: String, sessionId: String)(implicit
    hc: HeaderCarrier
  ): TrustEnvelope[GetTrustResponse] = EitherT {
    trustsService.getTrustInfo(identifier, internalId, sessionId).value.flatMap {
      case Right(response: TrustProcessedResponse)        =>
        populateLeadTrusteeAddress(response.getTrust) match {
          case JsSuccess(fixed, _) =>
            ...
            applyTransformations(identifier, internalId, fixed).value.map {
              ...
              case Right(JsError(errors))           =>
                logger.warn(
                  s"[$className][getTransformedData][SessionId: $sessionId] failed to apply transformations - $errors"
                )
          ...
          case JsError(errors) =>
            logger.warn(
              s"[$className][getTransformedData][SessionId: $sessionId] cannot populate lead trustee address - $errors"
            )
  ...
  private def applyTransformations(identifier: String, internalId: String, json: JsValue)(implicit
    hc: HeaderCarrier
  ): TrustEnvelope[JsResult[JsValue]] = EitherT {
  ...
  def applyDeclarationTransformations(identifier: String, internalId: String, json: JsValue)(implicit
    hc: HeaderCarrier
  ): TrustEnvelope[JsResult[JsValue]] = EitherT {
    repository.get(identifier, internalId, Session.id(hc)).value.map {
      case Right(None)                  =>
        logger.info(
          s"[$className][applyDeclarationTransformations][Session ID: ${Session.id(hc)}]" +
  ...
  def addNewTransform(identifier: String, internalId: String, newTransform: DeltaTransform)(implicit
    hc: HeaderCarrier
  ): TrustEnvelope[Boolean] = EitherT {
    repository
      .get(identifier, internalId, Session.id(hc))
      .value
      .map {
  ...
  def removeAllTransformations(identifier: String, internalId: String, sessionId: String): TrustEnvelope[Boolean] =
    EitherT {
  ...
  def removeTrustTypeDependentTransformFields(
    identifier: String,
    internalId: String,
    sessionId: String
  ): TrustEnvelope[Boolean] = EitherT {
    ...
    expectedResult.value.map {
      ...
      case Left(_)       =>
        logger.warn(
          s"[$className][removeTrustTypeDependentTransformFields][SessionId: $sessionId] " +
  ...
  private def handleTransformsForRemoveTrustTypeDependent(
    transforms: Option[ComposedDeltaTransform]
  ): ComposedDeltaTransform =
    transforms match {
      case Some(value) =>
        ComposedDeltaTransform(value.deltaTransforms.map {
   ...
   * </ol>
   */
  def removeOptionalTrustDetailTransforms(
    identifier: String,
    internalId: String,
    sessionId: String
  ): TrustEnvelope[Boolean] = EitherT {
    ...
    val expectedResult = for {
      ...
      updatedTransforms = transforms match {
                            case Some(value) =>
                              ComposedDeltaTransform(value.deltaTransforms.filter {
    ...
    expectedResult.value.flatMap {
      case Left(_)       =>
        logger.warn(
          s"[$className][removeOptionalTrustDetailTransforms][SessionId: $sessionId] " +
