...
class VariationService @Inject()(
        ...
        transformationService.populateLeadTrusteeAddress(originalResponse.getTrust) match {
          case JsSuccess(originalJson, _) =>
            ...
            case Right(e@JsError(errors)) =>
              ...
              logger.error(s"[$className][submitDeclaration][Session ID: ${Session.id(hc)}][UTR/URN: $identifier] " +
                s"Failed to transform trust info ${JsError.toJson(errors)}")

              Future.successful(Left(VariationFailureForAudit(
                InternalServerErrorResponse, "There was a problem transforming data for submission to ETMP"
              )))
  ...
  private def transformAndSubmit(identifier: String,
    ...
    declarationTransformer.transform(response, originalJson, declaration, localDateService.now) match {
      case JsSuccess(value, _) =>
        logger.info(s"[$className][transformAndSubmit][Session ID: ${Session.id(hc)}][UTR/URN: $identifier] " +
      ...
      case JsError(errors) =>
        ...
        logger.error(s"[$className][transformAndSubmit][Session ID: ${Session.id(hc)}][UTR/URN: $identifier] " +
  ...
  private def getCachedTrustData(identifier: String, internalId: String, sessionId: String): TrustEnvelope[TrustProcessedResponse] = EitherT {
    ...
    def processGetTrustResponse(response: GetTrustResponse, fbn: String): Either[TrustErrors, TrustProcessedResponse] = {
      response match {
        case tpr: TrustProcessedResponse if tpr.responseHeader.formBundleNo == fbn =>
          logger.info(s"[$className][processGetTrustResponse][Session ID: $sessionId][UTR/URN: $identifier] " +
            s"returning TrustProcessedResponse")
          Right(response.asInstanceOf[TrustProcessedResponse])

        case _: TrustProcessedResponse =>
          logger.warn(s"[$className][processGetTrustResponse][Session ID: $sessionId][UTR/URN: $identifier] " +
        ...
        case _ =>
          ...
          Left(VariationFailureForAudit(InternalServerErrorResponse, "Submission could not proceed, Trust data was not in a processed state"))
      }
    }
  ...
  private def submitVariationAndCheckForMigration(identifier: String, value: JsValue, internalId: String, sessionId: String)
    ...
    def migrateNonTaxableToTaxable(migrateToTaxable: Boolean,
                                   subscriptionId: String,
                                   identifier: String): TrustEnvelope[TaxEnrolmentSubscriberResponse] = EitherT {
      if (migrateToTaxable) {
        logger.info(s"[$className][migrateNonTaxableToTaxable][Session ID: ${Session.id(hc)}][UTR/URN: $identifier] " +
  ...
  private def submitVariation(
    ...
    trustsService.trustVariation(payload).value.map {
      case Right(response) =>
        logger.info(s"[$className][submitVariation][Session ID: ${Session.id(hc)}][UTR/URN: $identifier] trust submitted a variation")
