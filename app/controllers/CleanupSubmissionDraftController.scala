...
class CleanupSubmissionDraftController @Inject() (
  ...
  def removeDraft(draftId: String): Action[AnyContent] = identify.async { request =>
    submissionRepository.removeDraft(draftId, request.internalId).value.map {
      ...
      case Left(ServerError(message)) if message.nonEmpty =>
        logger.warn(
          s"[$className][removeDraft][Session ID: ${request.sessionId}] failed to remove draft. Message: $message"
        )
        InternalServerError
      case Left(_)                                        =>
        logger.warn(
          s"[$className][removeDraft][Session ID: ${request.sessionId}] an error occurred, failed to remove draft."
        )
        InternalServerError
    }
  }
  ...
  def reset(draftId: String, section: String, mappedDataKey: String): Action[AnyContent] = identify.async {
    implicit request =>
      submissionRepository.getDraft(draftId, request.internalId).value.flatMap {
        ...
        case Left(ServerError(message)) if message.nonEmpty =>
          logger.warn(
            s"[$className][reset][Session ID: ${request.sessionId}] cannot reset section $section. Message: $message"
          )
          Future.successful(InternalServerError)
        case Left(_)                                        =>
          logger.warn(
            s"[$className][reset][Session ID: ${request.sessionId}] an error occurred, cannot reset section $section"
          )
          Future.successful(InternalServerError)
      }
  }
    ...
    draft.draftData.transform {
      ...
      case JsSuccess(data, _) =>
        ...
        submissionRepository.setDraft(draftWithSectionReset).value.map {
          ...
          case Left(ServerError(message)) if message.nonEmpty =>
            logger.warn(
              s"[$className][reset][Session ID: ${request.sessionId}] failed to reset for $section. Message: $message"
            )
            InternalServerError
          case Left(_)                                        =>
            logger.warn(
              s"[$className][reset][Session ID: ${request.sessionId}] there was a problem, failed to reset for $section."
            )
            InternalServerError
        }
      case _: JsError         =>
  ...
  def removeRoleInCompany(draftId: String): Action[AnyContent] = identify.async { implicit request =>
    ...
    submissionRepository.getDraft(draftId, request.internalId).value.flatMap {
      ...
      case Left(ServerError(message)) if message.nonEmpty =>
        logger.warn(
          s"[$className][removeRoleInCompany][Session ID: ${request.sessionId}] " +
  ...
  def removeDeceasedSettlorMappedPiece(draftId: String): Action[AnyContent] = identify.async { implicit request =>
    val path: JsPath = JsPath \ "registration" \ "trust/entities/deceased"
    removeAtPath(draftId, path)
  }

  def removeLivingSettlorsMappedPiece(draftId: String): Action[AnyContent] = identify.async { implicit request =>
    val path: JsPath = JsPath \ "registration" \ "trust/entities/settlors"
    removeAtPath(draftId, path)
  }

  private def removeAtPath(draftId: String, path: JsPath)(implicit
    ...
    submissionRepository.getDraft(draftId, request.internalId).value.flatMap {
      case Right(Some(draft)) =>
        ...
        submissionRepository.setDraft(newDraft).value.map {
          ...
          case Left(ServerError(message)) if message.nonEmpty =>
            logger.warn(
              s"[$className][removeAtPath][Session ID: ${request.sessionId}] failed to set new draft data. Message: $message"
            )
            InternalServerError
          case Left(_)                                        =>
            logger.warn(
              s"[$className][removeAtPath][Session ID: ${request.sessionId}] there was a problem, failed to set new draft data."
            )
            InternalServerError
        }
      ...
      case Left(ServerError(message)) if message.nonEmpty =>
        logger.warn(
          s"[$className][removeAtPath][Session ID: ${request.sessionId}] cannot remove at path. Message: $message"
        )
        Future.successful(InternalServerError)
      case Left(_)                                        =>
        logger.warn(
          s"[$className][removeAtPath][Session ID: ${request.sessionId}] there was a problem, cannot remove at path"
        )
        Future.successful(InternalServerError)
    }
