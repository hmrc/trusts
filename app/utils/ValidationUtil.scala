...
trait ValidationUtil {
  ...
  def getTrusteesUtrWithIndex(trustees: List[TrusteeType]): List[(String, Int)] = {
    val utrList: List[(String, Int)] = trustees.zipWithIndex.flatMap { case (trustee, index) =>
  ...
  def getTrusteesNinoWithIndex(trustees: List[TrusteeType]): List[(String, Int)] = {
    val ninoList: List[(String, Int)] = trustees.zipWithIndex.flatMap { case (trustee, index) =>
  ...
  def getIndBenificiaryNinoWithIndex(indBenificiaries: List[IndividualDetailsType]): List[(String, Int)] = {
    val ninoList: List[(String, Int)] = indBenificiaries.zipWithIndex.flatMap { case (indv, index) =>
  ...
  def getSettlorNinoWithIndex(settlors: List[Settlor]): List[(String, Int)] = {
    val ninoList: List[(String, Int)] = settlors.zipWithIndex.flatMap { case (settlor, index) =>
  ...
  def getIndBenificiaryPassportNumberWithIndex(indBenificiaries: List[IndividualDetailsType]): List[(String, Int)] = {
    val passportNumberList: List[(String, Int)] = indBenificiaries.zipWithIndex.flatMap { case (indv, index) =>
      indv.identification.map { x =>
  ...
  def isNotTrust(currentTypeOfTrust: TypeOfTrust, expectedType: TypeOfTrust): Boolean =
    currentTypeOfTrust != expectedType

  def getSettlorPassportNumberWithIndex(settlor: List[Settlor]): List[(String, Int)] = {
    val passportNumberList: List[(String, Int)] = settlor.zipWithIndex.flatMap { case (indv, index) =>
      indv.identification.map { x =>
        x.passport.map(y => (y.number, index))
      }
    }.flatten
  ...
  def getSettlorUtrNumberWithIndex(settlorCompanies: List[SettlorCompany]): List[(String, Int)] = {
    val utrList: List[(String, Int)] = settlorCompanies.zipWithIndex.flatMap { case (settlorCompany, index) =>
