...
class SettlorDomainValidation(registration: Registration) extends ValidationUtil with Logging {
  ...
  def deceasedSettlorDobIsNotFutureDate: Option[TrustsValidationError] =
    getDeceasedSettlor(registration).flatMap { deceased =>
  ...
  def deceasedSettlorDoDIsNotAfterDob: Option[TrustsValidationError] =
    getDeceasedSettlor(registration).flatMap { deceased =>
      ...
      if (result.flatten.isDefined && result.flatten.get) {
        Some(TrustsValidationError(s"Date of death is after date of birth", s"/trust/entities/deceased/dateOfDeath"))
      } else {
        None
      }
    }
  ...
  def deceasedSettlorIsNotTrustee: Option[TrustsValidationError] =
    getDeceasedSettlor(registration).flatMap { deceased =>
      ...
      registration.trust.entities.trustees.flatMap { trustees =>
        ...
        if (deceasedNino.isDefined && trusteeNino.contains(deceasedNino)) {
          Some(
            TrustsValidationError(
              s"Deceased NINO is same as trustee NINO.",
  ...
  def deceasedSettlorIsNotBeneficiary: Option[TrustsValidationError] =
    getDeceasedSettlor(registration).flatMap { deceased =>
      val deceasedNino = deceased.identification.flatMap(_.nino)
      registration.trust.entities.beneficiary.individualDetails.flatMap { indBeneficiary =>
        ...
        if (deceasedNino.isDefined && beneficiaryNino.contains(deceasedNino)) {
          Some(
            TrustsValidationError(
              s"Deceased NINO is same as beneficiary NINO.",
              s"/trust/entities/deceased/identification/nino"
            )
          )
        } else {
  ...
  def deceasedSettlorIsNotProtector: Option[TrustsValidationError] =
    getDeceasedSettlor(registration).flatMap { deceased =>
      val deceasedNino = deceased.identification.flatMap(_.nino)
      registration.trust.entities.protectors.flatMap { protectors =>
        val protectorNino =
          protectors.protector.map(x => x.flatMap(y => y.identification.map(z => z.nino))).toList.flatten
        if (deceasedNino.isDefined && protectorNino.contains(deceasedNino)) {
          Some(
            TrustsValidationError(
              s"Deceased NINO is same as individual protector NINO.",
              s"/trust/entities/deceased/identification/nino"
            )
          )
        } else {
  ...
  def validateSettlor: Option[TrustsValidationError] = {
    ...
    trustType match {
      case None                                               => None
      case Some(TypeOfTrust.DeedOfVariationOrFamilyAgreement) =>
        validateDeedOfVariation(TypeOfTrust.DeedOfVariationOrFamilyAgreement)
  ...
  def livingSettlorDuplicateNino: List[Option[TrustsValidationError]] =
    getSettlorIndividuals(registration)
      .map { settlorIndividuals =>
        ...
        logger.info(
          s"[SettlorDomainValidation][livingSettlorDuplicateNino] Duplicate Ninos found: ${duplicatesNino.size} "
        )
        duplicatesNino.map { case (_, index) =>
          Some(
            TrustsValidationError(
              s"NINO is already used for another individual settlor.",
  ...
  def livingSettlorDobIsNotFutureDate: List[Option[TrustsValidationError]] =
    getSettlorIndividuals(registration)
      .map { settlorIndividuals =>
        val errors = settlorIndividuals.zipWithIndex.map { case (individualSettlor, index) =>
          isNotFutureDate(
            individualSettlor.dateOfBirth,
            s"/trust/entities/settlors/settlor/$index/dateOfBirth",
            "Date of birth"
          )
        }
        errors
  ...
  def livingSettlorDuplicatePassportNumber: List[Option[TrustsValidationError]] =
    getSettlorIndividuals(registration)
      .map { settlorIndividuals =>
        ...
        logger.info(
          s"[SettlorDomainValidation][livingSettlorDuplicatePassportNumber] Duplicate passport numbers found: ${duplicateUtrList.size} "
        )
        duplicateUtrList.map { case (_, index) =>
          Some(
            TrustsValidationError(
              s"Passport number is already used for another individual settlor.",
  ...
  def livingSettlorDuplicateUtr: List[Option[TrustsValidationError]] =
    getSettlorCompanies(registration)
      .map { settlorCompanies =>
        ...
        logger.info(
          s"[SettlorDomainValidation][livingSettlorDuplicateUtr] Number of Duplicate utrs found: ${duplicatePassportNumberList.size} "
        )
        duplicatePassportNumberList.map { case (_, index) =>
          Some(
            TrustsValidationError(
              s"Utr is already used for another settlor company.",
      ...
      .map { settlorCompanies =>
        ...
        utrList.map {
          case (utr, index) if trustUtr.contains(utr) =>
            Some(
              TrustsValidationError(
                s"Settlor company utr is same as trust utr.",
                s"/trust/entities/settlors/settlorCompany/$index/identification/utr"
              )
            )
          case _                                      =>
