...
class RequiredEntityDetailsForMigration {
  ...
  def areBeneficiariesCompleteForMigration(trust: JsValue): JsResult[MigrationStatus] = {
    ...
    def pickAtPathForBeneficiaryType[T <: Beneficiary[T]](`type`: String)(implicit rds: Reads[T]): JsResult[List[T]] =
    ...
    } yield (individuals, companies, trusts, charities, others, classesOfBeneficiary, larges) match {
  ...
  private def pickAtPathForEntityType[T <: MigrationEntity[T]](entity: String, `type`: String, trust: JsValue)(implicit
    rds: Reads[T]
  ): JsResult[List[T]] =
