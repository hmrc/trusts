...
class SetRecordedSpec extends IntegrationTestBase {

  "a set trust details recorded call" should {
    ...
    "return amended data in a subsequent 'get' call, for identifier '5174384721'" in assertMongoTest(application) {
      app =>
    ...
    def runTest(identifier: String, application: Application): Assertion = {
      ...
      val setRecordedProperty = JsBoolean(true)

      val amendRequest = FakeRequest(PUT, s"/trusts/trust-details/$identifier/recorded")
        .withBody(setRecordedProperty)
      ...
      val trustees = (contentAsJson(newResult) \ "trustRecorded").as[JsBoolean]
      trustees mustBe setRecordedProperty
