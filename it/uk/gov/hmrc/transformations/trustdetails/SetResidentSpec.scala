...
class SetResidentSpec extends AsyncFreeSpec with MustMatchers with MockitoSugar with IntegrationTestBase {

  "a set trust details resident call" - {
    ...
    def runTest(identifier: String, application: Application): Assertion = {
      ...
      val setResidentProperty = JsBoolean(true)

      val amendRequest = FakeRequest(PUT, s"/trusts/$identifier/trust-details/uk-resident")
        .withBody(setResidentProperty)
      ...
      val trustees = (contentAsJson(newResult) \ "trustUKResident").as[JsBoolean]
      trustees mustBe setResidentProperty
