...
class AmendDeceasedSettlorSpec extends AsyncFreeSpec with MustMatchers with MockitoSugar with IntegrationTestBase with ScalaFutures {
  ...
  "an amend deceased settlor call" - {

      val expectedGetAfterAmendJson: JsValue =
        JsonUtils.getJsonValueFromFile("it/trusts-integration-get-after-amend-deceased-settlor.json")
    ...
    def runTest(identifier: String, application: Application): Assertion = {
      ...
      val amendRequest = FakeRequest(POST, s"/trusts/settlors/amend-deceased/$identifier")
        .withBody(payload)
