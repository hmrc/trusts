...
class AmendCharityBeneficiarySpec extends AsyncFreeSpec with MustMatchers with MockitoSugar with IntegrationTestBase with ScalaFutures {
  ...
  "an amend charity beneficiary call" - {
    ...
    def runTest(identifier: String, application: Application): Assertion = {
      ...
      val amendRequest = FakeRequest(POST, s"/trusts/beneficiaries/amend-charity/$identifier/0")
        .withBody(payload)
