...
class AmendPropertyOrLandAssetSpec extends AsyncFreeSpec with MustMatchers with MockitoSugar with IntegrationTestBase with ScalaFutures {
  ...
  lazy val expectedSubsequentGetJson: JsValue = JsonUtils.getJsonValueFromFile("5MLD/NonTaxable/transforms/assets/propertyorland/amend-after-etmp-call.json")

  "an amend property or land asset call" - {
    ...
    def runTest(identifier: String, application: Application): Assertion = {
      ...
      val amendRequest = FakeRequest(POST, s"/trusts/assets/amend-property-or-land/$identifier/0")
        .withBody(payload)
