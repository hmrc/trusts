...
class AmendMoneyAssetSpec extends AsyncFreeSpec with MustMatchers with MockitoSugar with IntegrationTestBase with ScalaFutures {
  ...
  lazy val expectedSubsequentGetJson: JsValue = JsonUtils.getJsonValueFromFile("5MLD/NonTaxable/transforms/assets/money/amend-after-etmp-call.json")

  "an amend money asset call" - {
    ...
    def runTest(identifier: String, application: Application): Assertion = {
      ...
      val amendRequest = FakeRequest(POST, s"/trusts/assets/amend-money/$identifier/0")
        .withBody(payload)
