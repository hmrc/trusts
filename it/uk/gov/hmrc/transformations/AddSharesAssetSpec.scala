...
class AddSharesAssetSpec extends AsyncFreeSpec with MustMatchers with MockitoSugar with IntegrationTestBase {
  ...
  lazy val expectedSubsequentGetJson: JsValue = JsonUtils.getJsonValueFromFile("5MLD/NonTaxable/transforms/assets/shares/add-after-etmp-call.json")

  "an add shares asset call" - {

    val payload = Json.parse(
      """
        |{
        |  "orgName": "Another shares Ltd"
        |}
        |""".stripMargin)
    ...
    def runTest(identifier: String, application: Application): Assertion = {
      ...
      val addRequest = FakeRequest(POST, s"/trusts/assets/add-shares/$identifier")
        .withBody(payload)
