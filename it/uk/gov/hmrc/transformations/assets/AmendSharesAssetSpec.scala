...
class AmendSharesAssetSpec extends AsyncFreeSpec with MustMatchers with MockitoSugar with IntegrationTestBase with ScalaFutures {
  ...
  lazy val expectedSubsequentGetJson: JsValue = JsonUtils.getJsonValueFromFile("5MLD/NonTaxable/transforms/assets/shares/amend-after-etmp-call.json")

  "an amend shares asset call" - {

    val payload = Json.parse(
      """
        |{
        |  "orgName": "Changed shares Ltd"
        |}
        |""".stripMargin)
    ...
    def runTest(identifier: String, application: Application): Assertion = {
      ...
      val amendRequest = FakeRequest(POST, s"/trusts/assets/amend-shares/$identifier/0")
        .withBody(payload)
