...
class AddMoneyAssetSpec extends AsyncFreeSpec with MockitoSugar with IntegrationTestBase {
  ...
  lazy val expectedInitialGetJson: JsValue = NonTaxable5MLDFixtures.Trusts.newGetTransformedNonTaxableTrustResponse

  lazy val expectedSubsequentGetJson: JsValue = JsonUtils.getJsonValueFromFile("5MLD/NonTaxable/transforms/assets/money/add-after-etmp-call.json")

  "an add money asset call" - {

    val payload = Json.parse(
      """
        |{
        |  "assetMonetaryAmount": 999999999999
        |}
        |""".stripMargin)
