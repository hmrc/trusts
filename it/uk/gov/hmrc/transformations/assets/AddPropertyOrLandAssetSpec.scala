...
class AddPropertyOrLandAssetSpec extends AsyncFreeSpec with MustMatchers with MockitoSugar with IntegrationTestBase {
  ...
  lazy val expectedSubsequentGetJson: JsValue = JsonUtils.getJsonValueFromFile("5MLD/NonTaxable/transforms/assets/propertyorland/add-after-etmp-call.json")

  "an add property or land asset call" - {
    ...
    def runTest(identifier: String, application: Application): Assertion = {
      ...
      val addRequest = FakeRequest(POST, s"/trusts/assets/add-property-or-land/$identifier")
        .withBody(payload)
