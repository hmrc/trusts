...
class AmendOtherBeneficiarySpec extends FreeSpec with MustMatchers with MockitoSugar with TransformIntegrationTest with ScalaFutures {
  ...
  "an amend other beneficiary call" - {

    "must return amended data in a subsequent 'get' call" in {

      val expectedGetAfterAmendBeneficiaryJson: JsValue =
        JsonUtils.getJsonValueFromFile("trusts-integration-get-after-amend-other-beneficiary.json")
      ...
      running(application) {
        getConnection(application).map { connection =>
          ...
          val amendRequest = FakeRequest(POST, "/trusts/beneficiaries/amend-other/5174384721/0")
            .withBody(payload)
