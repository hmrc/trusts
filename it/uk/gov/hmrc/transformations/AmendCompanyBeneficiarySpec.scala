  ...
  "an amend company beneficiary call" - {
    ...
    "must return amended data in a subsequent 'get' call" in {
      ...
      val expectedGetAfterAmendBeneficiaryJson: JsValue =
        JsonUtils.getJsonValueFromFile("trusts-integration-get-after-amend-company-beneficiary.json")
      ...
      running(application) {
        getConnection(application).map { connection =>
          ...
          val amendRequest = FakeRequest(POST, "/trusts/beneficiaries/amend-company/5174384721/0")
            .withBody(payload)
