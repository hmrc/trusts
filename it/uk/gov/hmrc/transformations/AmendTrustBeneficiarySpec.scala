...
class AmendTrustBeneficiarySpec extends AsyncFreeSpec with MustMatchers with MockitoSugar with IntegrationTestBase with ScalaFutures {
  ...
  "an amend trust beneficiary call" - {
    ...
    def runTest(identifier: String, application: Application): Assertion = {
      ...
      val amendRequest = FakeRequest(POST, s"/trusts/beneficiaries/amend-trust/$identifier/0")
        .withBody(payload)
